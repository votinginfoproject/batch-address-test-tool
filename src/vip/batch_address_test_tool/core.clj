...
(defn retrieve-file
  ...
  (try
    (let [file-name   (get-in ctx [:input "fileName"])
          ...
          file        (file-store/fetch-file bucket-name file-name)]
...
(defn prepare-response
  ...
  (if (contains? ctx :error)
    ...
    (let [group (->group (get-in ctx [:input "fileName"]))
      ...
      (file-store/save-file bucket-name output-file-name output-file)
      (assoc ctx :results {:file-name output-file-name
                           :bucket-name bucket-name
                           :url (.toString (file-store/url-for-file bucket-name output-file-name))}))))
